...
# -------------------------
class BaseRoutine(Base):
    __tablename__ = "base_routines"

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)

    sheet_routine_id = Column(String, nullable=False, unique=True)  # RB001, RB002...
    ...
    items = relationship("BaseRoutineItem", back_populates="base_routine", cascade="all, delete-orphan")


# -------------------------
# BASE ROUTINE ITEMS (DB: public.base_routine_items)
# -------------------------
class BaseRoutineItem(Base):
    __tablename__ = "base_routine_items"

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    base_routine_id = Column(UUID(as_uuid=True), ForeignKey("base_routines.id", ondelete="CASCADE"), nullable=False)
    ...
    weight_base_kg = Column(Numeric, nullable=True)
    notes = Column(String, nullable=True)

    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False)

    base_routine = relationship("BaseRoutine", back_populates="items")

    __table_args__ = (
        UniqueConstraint("base_routine_id", "day_index", "order_index", name="ux_base_routine_items_unique_slot"),
    )


# -------------------------
# CLIENT ROUTINES (DB: public.client_routines)
# Snapshot de rutina base asignada a un cliente
# -------------------------
class ClientRoutine(Base):
    ...
    client_id = Column(UUID(as_uuid=True), ForeignKey("app_users.id", ondelete="CASCADE"), nullable=False)
    base_routine_id = Column(UUID(as_uuid=True), ForeignKey("base_routines.id"), nullable=False)

    assigned_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    active = Column(Boolean, nullable=False, default=True)

    items = relationship("ClientRoutineItem", back_populates="client_routine", cascade="all, delete-orphan")


# -------------------------
# CLIENT ROUTINE ITEMS (DB: public.client_routine_items)
# Copia exacta de base_routine_items al momento de asignar
# -------------------------
class ClientRoutineItem(Base):
    __tablename__ = "client_routine_items"
    ...
    client_routine = relationship("ClientRoutine", back_populates="items")
